...
if __name__ == '__main__':
    ...
    TEST_N_TAPS = 10
    TEST_N_TRAIN = 50000
    TEST_LATENCIES = [5, 20, 50]

    print("Executing objective_function with test parameters...")
    
    returned_metrics = objective_function(
        n_taps=TEST_N_TAPS, 
        n_train=TEST_N_TRAIN, 
        latency_list=TEST_LATENCIES,
    )

    print("\n--- RETURNED METRICS DICTIONARY ---")
    print(json.dumps(returned_metrics, indent=4))
    print("-----------------------------------")
